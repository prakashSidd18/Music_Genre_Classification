...
data = np.loadtxt('features_others.npy',delimiter=',',dtype=float)
data1 = np.loadtxt('mfcc_gaussian.npy', delimiter=',',dtype=float)
data = np.append(data1, data, 1)
print np.shape(data)
labels = np.loadtxt('labels_others.npy',delimiter=',',dtype=float)
labels.astype(int)
print np.shape(data)
...
#SVM Classifier using svc one vs one approach
...
# #SVM Linear Classifier one vs rest approach
# lsvc = svm.LinearSVC(random_state=0)
# lsvc = OneVsRestClassifier(lsvc)
# lclf = CalibratedClassifierCV(lsvc, cv=10)
# lclf.fit(train_data,train_label)
# predictions_lsvm = lclf.predict(test_data)
